		...
		if(!empty($_GPC['token'])) {
			...
			if(!empty($_GPC['nickname']) && empty($_GPC['openid'])) {
				...
				if(empty($user['openid'])) {
					message('没有找到昵称为 "'. $_GPC['nickname'] .'" 的用户', $this->createWebUrl('chatlog', array('acid' => $acid, 'nickname' => $_GPC['nickname'], 'openid' => $_GPC['openid'], 'starttime' => $_GPC['starttime'])), 'error', true);
				} else {
			...
			if(empty($user['openid'])) {
				message('请输入粉丝昵称或者粉丝openid', referer(), 'error', true);
			}
