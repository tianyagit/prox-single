...
function article_notice_all($filter = array(), $pindex = 1, $psize = 10) {
	...
	foreach ($notice as $key => $notice_val) {
		$notice[$key]['style'] = iunserializer($notice_val['style']);
		$notice[$key]['group'] = empty($notice_val['group']) ? array() : iunserializer($notice_val['group']);
		if (empty($_W['user']) && !empty($notice[$key]['group']) || !empty($_W['user']['groupid']) && !empty($notice[$key]['group']) && !in_array($_W['user']['groupid'], $notice[$key]['group'])) {
			unset($notice[$key]);
		}
	}
