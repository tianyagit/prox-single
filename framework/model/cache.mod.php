			...
			foreach ($menu['section'] as $section_name => $section) {
				...
				foreach ($section['menu']  as $permission_name => $sub_menu) {
					...
					$system_menu[$menu_name]['section'][$section_name]['menu'][$permission_name] = array(
						...
						'is_display' => (isset($sub_menu['is_display']) && (empty($sub_menu['is_display']) || (is_array($sub_menu['is_display']) && !in_array($account['type'], $sub_menu['is_display'])))) ? 0 : (isset($sub_menu_db['is_display']) ? $sub_menu_db['is_display'] : 1),
