 ...
 *   通知服务器生成小程序代码
 */
function wxapp_code_generate($version_id, $default = true) {
	global $_W;
	load()->classs('cloudapi');
	$api = new CloudApi();
...
function wxapp_code_default_appjson($version_id) {
	...
	if(empty($appjson)) {
		...
		//图片base64 转为本地路径
//		wxapp_code_convert_tablist($cloud_appjson);
		$appjson = $cloud_appjson;
...
function wxapp_code_path_to_base64($path) {
	...
	{
		if(starts_with($path, '//')) {
			$path = str_replace('//', '', $_W['sitescheme']).''.$path;
		}
