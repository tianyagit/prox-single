...
load()->table('account');
class XiongzhangappTable extends AccountTable {
 ...
 * @since version
 */
public function createXiongzhangappInfo($attr, $uid) {
	$name = $attr['name'];
...
private function createLog($uniacid, $uid) {
	if (empty($_W['isfounder'])) {
		...
		if (empty($user_info['usergroup_xiongzhangapp_limit'])) {
			pdo_update('account', array('endtime' => strtotime('+1 month', time())), array('uniacid' => $uniacid));
			pdo_insert('site_store_create_account', array('uid' => $uid, 'uniacid' => $uniacid, 'type' => ACCOUNT_TYPE_XIONGZHANGAPP_NORMAL));
		}
	}
