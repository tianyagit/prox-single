...
util.iconBrowser = function(callback){
	require(['fontawesome'], function(){
		...
		var modalobj = util.dialog('请选择图标', window.util.templates['fontawesome-content.tpl'], footer, {containerName:'icon-container'});
...
util.editor = function(elm, options){
	if(!elm &&　elm!='') return '';
		...
		UE.registerUI('myinsertvideo', function(editor, uiName) {
			editor.registerCommand(uiName, {
				execCommand: function() {
					...
					}, { fileSizeLimit: options.audio_limit * 1024, type: 'video', allowUploadVideo: options.allow_upload_video });
				}
			});
			var btn = new UE.ui.Button({
				name: '插入视频',
...
util.modal_message = function(title, msg, redirect, type, time) {
