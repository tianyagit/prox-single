...
angular.module('materialApp', []).controller('');
    $scope.context.group_sex = [
      {
        name: '\u7537',
        value: '1'
      },
    ...
    console.dir($scope.context.group);
        console.dir(dat);
          console.dir(data);
          console.dir($scope.context.activeItem);
          console.log('6666');
          console.dir($scope.context.activeItem);
          console.dir($scope.context.activeItem);
          console.dir($scope.context.activeItem);
...
angular.module('paymentApp', ['app']).value('config', { '': '' }).controller('paySetting', [
  '$scope',
  'config',
  '$timeout',
  function ($scope, config, $timeout) {
    $scope.credit = config.credit;
    $scope.delivery = config.delivery;
    $scope.wechat_facilitator = config.wechat_facilitator;
    $scope.wechat = config.wechat;
    $scope.baifubao = config.baifubao;
    $scope.line = config.line;
    $scope.alipay = config.alipay;
    $scope.unionpay = config.unionpay;
    ////支付宝测试
    $scope.tpost = function () {
      if ($scope.alipay.switch == 'true') {
        if (confirm('\u4f60\u53ef\u80fd\u9700\u8981\u4fee\u6539\u60a8\u7684\u6d4f\u89c8\u5668 User-Agent \u6765\u6a21\u62df\u624b\u673a\u6d4f\u89c8\u624d\u80fd\u6b63\u5e38\u6d4b\u8bd5, \u8bf7\u786e\u8ba4\u5df2\u7ecf\u4fee\u6539\u597d.')) {
          $scope.alipay.test = 'true';
          $timeout(function () {
            $scope.submit();
          });
        }
    ...
    $scope.$watch('wechat_facilitator.switch', function (newvalue, oldvalue) {
      if (newvalue == 'true') {
    ...
    $scope.tokenGen = function (name) {
      ...
      if (name == 'wechat_facilitator.signkey') {
        $scope.wechat_facilitator.signkey = token;
      }
      if (name == 'wechat.apikey') {
        $scope.wechat.apikey = token;
      }
    };