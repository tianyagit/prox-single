...
if ($do == 'post') {
	...
	if (checksubmit('submit')) {
		...
		itoast('用户组更新成功！', url('user/foundergroup'), 'success');
	}
	template('user/founder-group-post');
}
...
if ($do == 'del') {
	...
	if(!empty($result)){
		itoast('删除成功！', url('user/foundergroup/'), 'success');
	}else {
		itoast('删除失败！请稍候重试！', url('user/foundergroup'), 'error');
	}
	exit;
}