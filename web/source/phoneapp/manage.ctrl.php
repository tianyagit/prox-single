...
if ($do == 'save') {
	...
	if (empty($uniacid) && empty($account_info['phoneapp_limit']) && !user_is_founder($_W['uid'])) {
		iajax(-1, '创建APP个数已满', url('account/privileges', array('type' => 'phoneapp')));
	}
	...
	if (empty($uniacid) && empty($version_id)) {
		...
		iajax(0, '修改成功', url('account/privileges', array('type' => 'phoneapp')), array('uniacid' => $uniacid));
	} else {
		unset($data['name']);
	...
	if (!empty($result)) {
		iajax(0, '创建成功', url('account/privileges', array('type' => 'phoneapp')));
	}
