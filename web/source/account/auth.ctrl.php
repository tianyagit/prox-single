...
if ($do == 'forward') {
	if (empty($_GPC['auth_code'])) {
		message('授权登录失败，请重试', url('account/system_display'), 'error');
	}
	...
	if (is_error($account_info)) {
		message('授权登录新建公众号失败，请重试', url('account/system_display'), 'error');
	}
	if (!empty($_GPC['test'])) {
	...
	message('授权登录成功', url('account/system_display', array('type' => '3')), 'success');
} elseif ($do == 'confirm') {
	...
	message('更改公众号授权接入成功', url('account/system_display', array('type' => '3')), 'success');
} elseif ($do == 'ticket') {
