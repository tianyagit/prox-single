...
if ($do == 'home') {
	...
	if (!empty($last_uniacid) && $last_uniacid != $_W['uniacid']) {
		uni_account_switch($last_uniacid, '', 'wxapp');
	}
	...
	if (!empty($module_name) && !empty($version_id)) {
		...
		if (!empty($module_info['account']['uniacid'])) {
			...
			uni_account_switch($version_info['uniacid'], $url, 'wxapp');
		}
	}
	wxapp_update_last_use_version($uniacid, $version_id);
	uni_account_switch($uniacid, url('wxapp/version/home', array('version_id' => $version_id)), 'wxapp');
	exit;
} elseif ($do == 'rank') {
